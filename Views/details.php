            ...
            <div class="row align-items-center">
                ...
                <div class="col-lg-6 col-md-12 col-12">
                    <div class="product-info">
                        <h2 class="title">{{$productDetails[0]->{product_name} }}</h2>
                        ...
                        <p class="info-text">{{$productDetails[0]->{product_desc} }}</p>
                        <!--size buttons-->
                        <div class="row">
                            ...
                            <@ $i=0; $checked="" ; $firstColorID="" ; foreach($colorPhotos AS $item): $i++; if($i==1){ $checked="checked" ; $firstColorID=$item->color_id;
                                ...
                                <div class='col text-center image_space' onclick="selectColor('{{$item->color_id}}');">
                                    ...
                                    <label for="{{$item->color_id}}">
                                        <img src="{{baseurl}}/assets/images/products/{{$item->photo_url}}" alt="{{$item->{color_name} }}" style="max-height:75px!important;">
                ...
                <div class="row">
                    <div class="col-lg-6 col-12">
                        <div class="info-body custom-responsive-margin">
                            ...
                            <p>{{$productDetails[0]->{product_full} }}</p>
                        </div>
                    </div>
                    <div class="col-lg-6 col-12">
                        <div class="info-body">
                            ...
                            <ul class="normal-list">
                                <li><span>{{weight}}:</span> {{$productDetails[0]->weight}} kg. </li>
                                <li><span>{{category}}:</span> {{$productDetails[0]->{cat_name} }}</li>
                                <li><span>{{product_title}}:</span> {{$productDetails[0]->{product_name} }}</li>
                                <li><span>{{brand}}:</span> {{$productDetails[0]->brand_name}} </li>
                            </ul>
                        </div>
                    </div>
                </div>
        ...
        $.get("{{baseurl}}/product_size_ajax/{{$productID}}/" + colorID, function(data, status) {
            ...
            $.get("{{baseurl}}/photo_list_ajax/{{$productID}}/" + colorID, function(data, status) {
                ...
                $.each(data, function(key, value) {
                    i++;
                    if (i == 1) {
                        $selectMain.append(`<img src="{{baseurl}}/assets/images/products/` + value.photo_url + `" style="max-height:600px!important;" id="current" alt="#">`);
                    }
                    $select.append(`<img src="{{baseurl}}/assets/images/products/` + value.photo_url + `" class="img" style="max-height:100px!important;" alt="#">`);
    ...
    function selectColor(colorID) {
        ...
        $.get("{{baseurl}}/photo_list_ajax/{{$productID}}/" + colorID, function(data, status) {
            ...
            $.each(data, function(key, value) {
                ...
                $select.append(`<img src="{{baseurl}}/assets/images/products/` + value.photo_url + `" style="max-height:100px!important;" class="img" alt="#">`);

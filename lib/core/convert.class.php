...
class convert
{
        ...
        if (count($arr) > 0) {
            ...
            foreach ($arr as $key => $value) {
                ...
                if (is_array($value)) {
                    ...
                    $parts[] = $str . self::array2json($value);
                } else {
    ...
    public function objectSearch($productList, $searchKey, &$searchedValue){
        $neededObject = array_filter(
            $productList,
            function ($e) use (&$searchedValue, $searchKey) {
                return $e->{$searchKey} == $searchedValue;
            }
        );
        return $neededObject;
    }

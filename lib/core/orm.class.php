...
class orm
{
    ...
    public function update()
    {
        ...
        $string = rtrim($string, ",");
        $updateQuery = "UPDATE " . $this->table . " SET " . $string . " " . $filter . "";
        //echo $updateQuery;
        if (project_mode == "dev") {
    ...
    public function record()
    {
        ...
        if (!empty($this->insertColumns) && !empty($this->insertValues)) {
            ...
            $inserQuery = "INSERT INTO " . $this->table . " (" . $columnString . ") VALUES (" . $valueString . ")";
            if (project_mode == "dev") {
                echo "<pre>" . $inserQuery . "</pre>";
            }
            $this->mysqli->query("INSERT INTO " . $this->table . " (" . $columnString . ") VALUES (" . $valueString . ")");
    ...
    public function delete()
    {
        ...
        $deleteQuery = "DELETE FROM ".$this->table." WHERE ".$this->filter."";
    ...
    public function paginationLimit($num){
        $end = "";
    ...
    public function pagination($num)
    {
        ...
        if ($this->records % $num == 0) {
            $total = intval(($this->records - 1) / $num);
        }else{
            $total = intval(($this->records - 1) / $num) + 1;
        }
        //echo $this->records;
        ...
        while ($row = $result->fetch_object()) {
            ...
            if (!empty($row)) {
                ...
                if ($this->tree_check_parents($row->{$this->treeCatID}) == true) {
                    ...
                    $menu .= treeMenuStartTag . "<a href='" . baseurl . $this->treeUrl . $row->{$this->treeCatID} . "/'>" . $row->{$this->treeName} . "</a>";
